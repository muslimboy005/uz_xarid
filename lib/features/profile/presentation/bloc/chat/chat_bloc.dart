      ...
      if (!emit.isDone) {
        if (result.isRight) {
          ...
          if (event.page == 1) {
            ...
            final newChatRoomId = serverMessages.isNotEmpty
                ? serverMessages.first.chatRoom
                : state.chatRoomId;

            // 1. Detect highest server ID
            final highestServerId = serverMessages.fold<int>(
              0,
              (max, m) => m.id > max ? m.id : max,
            );

            // 2. Identify local messages that are newer or optimistic
            final localPreserved = currentMessages
                .where((m) => m.id < 0 || m.id > highestServerId)
                .toList();
            ...
            if (isPolling && currentMessages.isNotEmpty) {
              final idsChanged =
                  currentMessages.length != mergedResults.length ||
  ...
  Future<void> _onSendChatMessage(
    ...
    final optimisticMessage = ChatMessageModel(
      ...
      files: event.filePaths
          .map(
            (path) => ChatFileModel(id: -1, file: path, fileUrlDisplay: path),
          )
          .toList(),
    ...
    try {
      ...
      if (!emit.isDone) {
        if (result.isRight) {
          ...
          final rolledBackMessages = List<ChatMessageModel>.from(
            state.messages,
          );
