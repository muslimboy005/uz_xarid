...
class MyListingsRepositoryImpl implements MyListingsRepository {
  ...
  @override
  Future<Either<Failure, AdLimitInfoDto>> getLimitInfo() async {
    try {
      final response = await dio.get(ApiUrls.adLimitInfo);
      final raw = response.data;
      if (raw is! Map<String, dynamic>) {
        return Left(ServerFailure(message: 'Noto\'g\'ri javob'));
      }
      final status = raw['status'] as bool? ?? false;
      final data = raw['data'];
      if (!status || data is! Map<String, dynamic>) {
        return Left(ServerFailure(message: 'Ma\'lumot olinmadi'));
      }
      return Right(AdLimitInfoDto.fromJson(data));
    } on DioException catch (e) {
      final msg = e.response?.data is Map
          ? (e.response?.data as Map)['message'] ??
                (e.response?.data as Map)['detail']
